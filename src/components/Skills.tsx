...
import { Compass, Database, Globe, Palette, Terminal } from "lucide-react";
        ...
        <div className="relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {skills.map(({ name, icon: Icon, level }) => (
              <div
                key={name}
                className="bg-stone-900 rounded-lg p-6 transform hover:-translate-y-2 transition-all duration-300"
              >
                ...
                <div className="h-2 bg-stone-700 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${level}%` }} // Perbaikan disini
                  />
                </div>
                <span className="text-stone-400 text-sm mt-2 block">
                  {level}%
                </span>
              </div>
            ))}
